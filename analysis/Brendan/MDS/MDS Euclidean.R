...
ggplot(data = mds.data_genes2, aes(x=X, y=Y, label = Sample))+
  ...
  ggtitle("MDS Log fold changes- Genes")
...
#distance - Euclidean 
d_genes <- dist(scale(t(expressed.trans),center = TRUE, scale = TRUE), method = "euclidean") # for scaled values
d_genes <- dist(t(expressed.trans), method = "euclidean")
...
ggplot(data = mds.data_genes2, aes(x=X, y=Y, label = Sample))+
  ...
  xlab(paste("MDS1 - ", mds.var.per_genes2[1], "%", sep = ""))+
  ylab(paste("MD2 - ", mds.var.per_genes2[2], "%", sep = ""))+
  ggtitle("MDS Log fold changes- Transcripts")

rm(d_genes,log2.data.matrix.genes,log2.distance.matrix.genes,mds.data_genes,mds.data_genes2,mds.stuff_genes,mds.values_genes,mds.values_genes2)
rm(i,j,mds.var.per_genes,mds.var.per_genes2,mds_genes)