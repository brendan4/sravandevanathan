...
genes.PCA <- prcomp(t(na.omit(expressed.genes)),scale = FALSE)
summary(genes.PCA)
...
barplot(genes.PCA.var.per, main = "Scree Plot: Gene", 
...
gene_score_ranked <- sort(abs(genes.PCA$rotation[,1]), decreasing = TRUE)
